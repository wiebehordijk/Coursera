...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeeds = columnNames.filter(name => List("t01", "t03", "t11", "t1801", "t1803").exists(name.startsWith(_))).map(new Column(_))
    val workingActivities = columnNames.filter(name => List("t05", "t1805").exists(name.startsWith(_))).map(new Column(_))
    val otherActivities = columnNames.filter(name => List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
      .exists(name.startsWith(_)) && !List("t1801", "t1803", "t1805").exists(name.startsWith(_))).map(new Column(_))
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    val primary: TypedColumn[TimeUsageRow, Double] = round(typed.avg[TimeUsageRow](_.primaryNeeds), 1).as[Double]
    val work: TypedColumn[TimeUsageRow, Double] = round(typed.avg[TimeUsageRow](_.work), 1).as[Double]
    val other: TypedColumn[TimeUsageRow, Double] = round(typed.avg[TimeUsageRow](_.other), 1).as[Double]

    summed.groupByKey(r => (r.working, r.sex, r.age))
      .agg(primary, work, other)
//      .map{ case Row((working: String, sex: String, age: String), primaryNeeds: Double, work: Double, other: Double) =>
