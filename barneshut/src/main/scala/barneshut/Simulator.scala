...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def computeBoundaries(bodies: GenSeq[Body]): Boundaries = timeStats.timed("boundaries") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.aggregate(new Boundaries)(updateBoundaries, mergeBoundaries)
  }

  def computeSectorMatrix(bodies: GenSeq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
  ...
  def updateBodies(bodies: GenSeq[Body], quad: Quad): GenSeq[Body] = timeStats.timed("update") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.map(_.updated(quad))
  }

  def eliminateOutliers(bodies: GenSeq[Body], sectorMatrix: SectorMatrix, quad: Quad): GenSeq[Body] = timeStats.timed("eliminate") {
    ...
    val parBodies = bodies.par
    parBodies.filter(!outliers(_))
  }

  def step(bodies: GenSeq[Body]): (GenSeq[Body], Quad) = {
