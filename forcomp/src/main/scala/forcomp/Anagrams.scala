...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (w groupBy (e => e.toLower) mapValues (l => l.length) toList).sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(List())
    case (char, num) :: rest => {
      for (c <- combinations(rest); i <- 0 to num) yield if (i == 0) c else (char, i) :: c
    }.toList
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    for ((char, num) <- x if num > ymap(char)) yield (char, (num - ymap(char)))
  }
