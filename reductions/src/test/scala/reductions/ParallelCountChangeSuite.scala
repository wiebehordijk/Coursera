...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("parCountChange should return 1 when money == 0") {
    def check(coins: List[Int]) =
      assert(parCountChange(0, coins, totalCoinsThreshold(coins.sum)) == 1,
        s"countChang(0, _) should be 1")
  ...
  test("Money threshold works") {
    def check(money: Int, totalMoney: Int, expected: Boolean) =
      assert(moneyThreshold(totalMoney)(money, List()) === expected,
        s"moneyThreshold($money, $totalMoney) should be $expected")
